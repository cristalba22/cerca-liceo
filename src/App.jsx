...
import { createAdminScreen } from './screens/AdminScreen'
import { createAuthScreens } from './screens/AuthScreens'
import { createMerchantScreens } from './screens/MerchantScreens'
import { createPublicScreens } from './screens/PublicScreens'
...
const sharedPublicScreens = createPublicScreens({
  sections, categories, liceoMapEmbedUrl, liceoMapUrl, hasBusinessPin, getBusinessMapUrl,
  makeWhatsAppUrl, makeInstagramUrl, getBusinessMenu, hasBusinessPublicAddress,
  isFounderPlanActive, buildCercaWhatsAppMessage, getOpenStatus, getOfferOpenStatus,
  getOfferWhatsappUrl, MAX_MENU_ITEMS, MENU_SECTION_SIZE, menuCatalogSections,
  imageSurfaceProps, ThemeToggle,
})
const { DirectoryScreen, BusinessCard, BusinessDetailScreen, WelcomeScreen, DetailScreen, OfferCard } = sharedPublicScreens

const { PublishScreen, MerchantFirstLocalScreen, MyPostsScreen } = createMerchantScreens({
  sections, weekDays, commerceCategories, parseMapCoordinates, hasBusinessPin, getBusinessMapUrl,
  RealLocationPicker, isUploadedImage, isAndroidCompatMode, formatSchedule,
  normalizeArgentineWhatsapp, isValidArgentineWhatsapp, makeWhatsAppUrl,
  hasBusinessPublicAddress, isFounderPlanActive, isFounderPlanRequested, getFounderDaysLeft,
  isFounderExpiringSoon, isOfferExpired, isOfferPaused, isOfferActiveNow, getOfferDaysLeft,
  MAX_MENU_ITEMS, MENU_SECTION_SIZE, menuCatalogSections, createMenuSlot, ensureMenuSlots,
  buildFilledMenuSections, buildLocalDraft, imageSurfaceProps, readCompressedImage,
  ThemeToggle, HomeReturnStrip, ContactFooter, BusinessCard,
})

const { AdminScreen } = createAdminScreen({
  sections, commerceCategories, makeWhatsAppUrl, hasBusinessPublicAddress, isFounderPlanActive,
  isFounderPlanRequested, isFounderPlanExpired, isFounderExpiringSoon, isOfferExpired,
  isOfferPaused, isOfferActiveNow, getOfferDaysLeft, hasRealBusinessPhoto, isRecentBusiness,
  ThemeToggle,
})

const { LoginScreen, ForgotPasswordScreen, ResetPasswordScreen, ProfileScreen, PrivacyScreen, RegisterScreen } = createAuthScreens({
  sections, commerceCategories, parseMapCoordinates, hasBusinessPin, getBusinessMapUrl,
  RealLocationPicker, isAndroidCompatMode, normalizeArgentineWhatsapp, isValidArgentineWhatsapp,
  makeWhatsAppUrl, hasBusinessPublicAddress, isFounderPlanActive, isFounderPlanRequested,
  ThemeToggle, ContactFooter, cercaApi,
})
